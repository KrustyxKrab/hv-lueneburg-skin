...
if ( ! class_exists( 'TopscorerRestAPI' ) ) {
	/**
	 * Rest API class with configuration
	 */
	class TopscorerRestAPI {
		private static $instance;
		private $namespace;
		
		public function __construct() {
			// Init variables
			$this->set_namespace( 'topscorer/v1' );
			
			// Localize theme's main js script with rest variable
			add_filter( 'topscorer_filter_localize_main_js', array( $this, 'localize_script' ) );
		...
		function localize_script( $global ) {
			...
			return apply_filters( 'topscorer_filter_rest_api_global_variables', $global, $this->get_namespace() );
		}
		
		function register_rest_api_route() {
			$routes = apply_filters( 'topscorer_filter_rest_api_routes', array() );
