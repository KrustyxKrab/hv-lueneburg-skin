...
if ( ! function_exists( 'topscorer_add_rest_api_pagination_global_variables' ) ) {
	/**
	 ...
	 * @return array
	 */
	function topscorer_add_rest_api_pagination_global_variables( $global, $namespace ) {
	...
	add_filter( 'topscorer_filter_rest_api_global_variables', 'topscorer_add_rest_api_pagination_global_variables', 10, 2 );
}

if ( ! function_exists( 'topscorer_add_rest_api_pagination_route' ) ) {
	/**
	 ...
	 * @return array
	 */
	function topscorer_add_rest_api_pagination_route( $routes ) {
		$routes['pagination'] = array(
			'route'    => 'get-posts',
			'methods'  => WP_REST_Server::READABLE,
			'callback' => 'topscorer_get_new_posts',
			'args'     => array(
				'options' => array(
					'required'          => true,
					...
					'description'       => esc_html__( 'Options data is array with all selected shortcode parameters value', 'topscorer' ),
				),
			),
		);
	...
	add_filter( 'topscorer_filter_rest_api_routes', 'topscorer_add_rest_api_pagination_route' );
}

if ( ! function_exists( 'topscorer_get_new_posts' ) ) {
	/**
	 * Function that load new posts for pagination functionality
	 *
	 * @return void
	 */
	function topscorer_get_new_posts() {

		// phpcs:ignore WordPress.Security.NonceVerification
		if ( ! isset( $_GET ) || empty( $_GET ) ) {
			topscorer_get_ajax_status( 'error', esc_html__( 'Get method is invalid', 'topscorer' ) );
		} else {
			...
			if ( ! empty( $options ) ) {
				...
				$query_args = topscorer_get_query_params( $options );
				...
				if ( isset( $options['behavior'] ) ) {
					...
					echo apply_filters( 'topscorer_filter_masonry_template_part_pagination', topscorer_get_template_part( 'masonry', 'templates/sizer-gutter', '', $options['behavior'] ) );
				}

				// Variable name is function name - escaped no need.
				// phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
				echo apply_filters( "topscorer_filter_{$get_template_part}", $get_template_part( $module . '/' . $shortcode, 'templates/loop', '', $options ) );

				$html = ob_get_contents();

				ob_end_clean();

				topscorer_get_ajax_status ( 'success', esc_html__ ( 'Items are loaded', 'topscorer' ), $html );
			} else {
				topscorer_get_ajax_status( 'error', esc_html__( 'Options are invalid', 'topscorer' ) );
			}
		}
	}
}

if ( ! function_exists( 'topscorer_get_query_params' ) ) {
	/**
	 ...
	 * @return array
	 */
	function topscorer_get_query_params( $params ) {
		...
		return apply_filters( 'topscorer_filter_query_params', $args, $params );
	}
}

if ( ! function_exists( 'topscorer_get_pagination_data' ) ) {
	/**
	 ...
	 * @return array
	 */
	function topscorer_get_pagination_data( $plugin, $module, $shortcode, $post_type, $params ) {
		$data = array();
		...
		if ( ! empty( $post_type ) && ! empty( $params ) ) {
			...
			if ( isset( $params['space'] ) && ! empty( $params['space'] ) ) {
				$params['space_value'] = topscorer_get_space_value( $params['space'] );
			}

			$data = wp_json_encode( array_filter( array_merge( $additional_params, $params ), 'topscorer_validate_pagination_data' ) );
		}
...
if ( ! function_exists( 'topscorer_validate_pagination_data' ) ) {
	/**
	 ...
	 * @return mixed
	 */
	function topscorer_validate_pagination_data( $value ) {
...
if ( ! function_exists( 'topscorer_add_link_pages_after_content' ) ) {
	/**
	 * Function which add pagination for blog single and page
	 */
	function topscorer_add_link_pages_after_content() {

		$args_pages = array(
			'before'      => '<div class="qodef-single-links qodef-m"><span class="qodef-m-single-links-title">' . esc_html__( 'Pages: ', 'topscorer' ) . '</span>',
			'after'       => '</div>',
			'link_before' => '<span>',
			'link_after'  => '</span>',
			'pagelink'    => '%',
	...
	add_action( 'topscorer_action_after_blog_single_content', 'topscorer_add_link_pages_after_content' );
	add_action( 'topscorer_action_after_page_content', 'topscorer_add_link_pages_after_content' );
}
