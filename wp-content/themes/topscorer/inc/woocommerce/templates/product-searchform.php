...
<form role="search" method="get" class="qodef-woo-product-search" action="<?php echo esc_url( home_url( '/' ) ); ?>">
    <label class="screen-reader-text"
            for="woocommerce-product-search-field-<?php echo isset( $index ) ? absint( $index ) : 0; ?>"><?php esc_html_e( 'Search for:', 'topscorer' ); ?></label>
    <div class="qodef-search-form-inner clear">
        <input type="search" id="woocommerce-product-search-field-<?php echo isset( $index ) ? absint( $index ) : 0; ?>"
                class="qodef-search-form-field" placeholder="<?php echo esc_attr_e( 'Search', 'topscorer' ); ?>"
