...
abstract class TopScorerCoreListShortcode extends QodeFrameworkShortcode {
	private $post_type;
	...
	public function map_list_options( $params = array() ) {
		...
		$this->set_option( array(
			'field_type'    => 'select',
			'name'          => 'behavior',
			'title'         => esc_html__( 'List Behavior', 'topscorer-core' ),
			'options'       => topscorer_core_get_select_type_options_pool( 'list_behavior', false, $exclude_behavior ),
		...
		if ( empty( $exclude_behavior ) || ( ! empty( $exclude_behavior ) && ! in_array( 'masonry', $exclude_behavior ) ) ) {
			$this->set_option( array(
				...
				'title'         => esc_html__( 'Images Proportion', 'topscorer-core' ),
				'options'       => array(
					''      => esc_html__( 'Original', 'topscorer-core' ),
		...
		if ( empty( $exclude_option ) || ( ! empty( $exclude_option ) && ! in_array( 'images_proportion', $exclude_option ) ) ) {
			$this->set_option( array(
				...
				'default_value' => 'full',
				'title'         => esc_html__( 'Images Proportion', 'topscorer-core' ),
				'options'       => topscorer_core_get_select_type_options_pool( 'list_image_dimension', false ),
				'group'         => $group,
				'dependency'    => array(
					'show' => array(
						'behavior' => array(
			...
			$this->set_option( array(
				...
				'title'       => esc_html__( 'Custom Image Width', 'topscorer-core' ),
				'description' => esc_html__( 'Enter image width in px', 'topscorer-core' ),
				'group'       => $group,
				'dependency'  => array(
					'show' => array(
			...
			$this->set_option( array(
				...
				'title'       => esc_html__( 'Custom Image Height', 'topscorer-core' ),
				'description' => esc_html__( 'Enter image height in px', 'topscorer-core' ),
				'group'       => $group,
				'dependency'  => array(
					'show' => array(
						'images_proportion' => array(
		...
		$this->set_option( array(
			...
			'options'       => topscorer_core_get_select_type_options_pool( 'columns_number', true, $exclude_columns, $include_columns ),
			'default_value' => '3',
			'group'         => $group,
			'dependency'    => array(
				'hide' => array(
		...
		$this->set_option( array(
			...
			'title'         => esc_html__( 'Columns Responsive', 'topscorer-core' ),
			'options'       => topscorer_core_get_select_type_options_pool( 'columns_responsive' ),
		...
		$this->set_option( array(
			'field_type'    => 'select',
			'name'          => 'columns_1440',
			'title'         => esc_html__( 'Columns Number 1367px - 1440px', 'topscorer-core' ),
		...
		$this->set_option( array(
			'field_type'    => 'select',
			'name'          => 'columns_1366',
			'title'         => esc_html__( 'Columns Number 1025px - 1366px', 'topscorer-core' ),
			'options'       => topscorer_core_get_select_type_options_pool( 'columns_number' ),
			'default_value' => '3',
			'dependency'    => array(
				'show' => array(
		...
		$this->set_option( array(
			'field_type'    => 'select',
			'name'          => 'columns_1024',
			'title'         => esc_html__( 'Columns Number 769px - 1024px', 'topscorer-core' ),
			'options'       => topscorer_core_get_select_type_options_pool( 'columns_number' ),
			'default_value' => '3',
			'dependency'    => array(
				'show' => array(
		...
		$this->set_option( array(
			'field_type'    => 'select',
			'name'          => 'columns_768',
			'title'         => esc_html__( 'Columns Number 681px - 768px', 'topscorer-core' ),
			'options'       => topscorer_core_get_select_type_options_pool( 'columns_number' ),
			'default_value' => '3',
			'dependency'    => array(
				'show' => array(
		...
		$this->set_option( array(
			'field_type'    => 'select',
			'name'          => 'columns_680',
			'title'         => esc_html__( 'Columns Number 481px - 680px', 'topscorer-core' ),
			'options'       => topscorer_core_get_select_type_options_pool( 'columns_number' ),
			'default_value' => '3',
			'dependency'    => array(
				'show' => array(
		...
		$this->set_option( array(
			'field_type'    => 'select',
			'name'          => 'columns_480',
			'title'         => esc_html__( 'Columns Number 0 - 480px', 'topscorer-core' ),
			'options'       => topscorer_core_get_select_type_options_pool( 'columns_number' ),
			'default_value' => '3',
			'dependency'    => array(
				'show' => array(
		...
		if ( empty( $exclude_behavior ) || ( ! empty( $exclude_behavior ) && ! in_array( 'justified-gallery', $exclude_behavior ) ) ) {
			$this->set_option( array(
				...
				'title'      => esc_html__( 'Row Height', 'topscorer-core' ),
				'dependency' => array(
					'show' => array(
						'behavior' => array(
							'values'        => 'justified-gallery',
			...
			$this->set_option( array(
				...
				'title'      => esc_html__( 'Max Row Height', 'topscorer-core' ),
				'dependency' => array(
					'show' => array(
						'behavior' => array(
							'values'        => 'justified-gallery',
			...
			$this->set_option( array(
				...
				'title'      => esc_html__( 'Last Row Treshold', 'topscorer-core' ),
				'dependency' => array(
					'show' => array(
						'behavior' => array(
							'values'        => 'justified-gallery',
	...
	public function get_list_item_image_dimension( $atts ) {
		...
		if ( ! empty( $atts['behavior'] ) && $atts['behavior'] == 'masonry' && ! empty( $atts['masonry_images_proportion'] ) && $atts['masonry_images_proportion'] == 'fixed' ) {
			...
			$image_dimension              = topscorer_core_get_custom_image_size_meta( 'meta-box', $masonry_image_dimension_name, get_the_ID() );
		}

		if ( ! empty( $atts['behavior'] ) && in_array( $atts['behavior'], array( 'columns', 'slider' ) ) ) {
			$image_dimension = array(
				'size'  => $atts['images_proportion'],
				'class' => topscorer_core_get_custom_image_size_class_name( $atts['images_proportion'] )
			);
		}
	...
	public function get_slider_data( $atts, $include = array() ) {
		...
		$data['spaceBetween']   = isset( $atts['space'] ) ? topscorer_core_get_space_value( $atts['space'] ) : 0;
		...
		$data['transitionEffect']      = isset( $atts['transition_effect'] ) ? $atts['transition_effect']: 'slide';
	...
	public function map_query_options( $params = array() ) {
		$group                = isset( $params['group'] ) ? $params['group'] : esc_html__( 'Query', 'topscorer-core' );
		...
		$this->set_option( array(
			'field_type' => 'text',
			'name'       => 'posts_per_page',
			'title'      => esc_html__( 'Posts per Page', 'topscorer-core' ),
			'default'    => '-1',
			'group'      => $group
		) );
		$this->set_option( array(
			'field_type'    => 'select',
			'name'          => 'orderby',
			'title'         => esc_html__( 'Order By', 'topscorer-core' ),
			'options'       => topscorer_core_get_select_type_options_pool( 'order_by', true, $exclude_order_by, $include_order_by ),
			'default_value' => 'date',
			'group'         => $group
		) );
		$this->set_option( array(
		...
		$this->set_option( array(
			...
			'title'      => esc_html__( 'Additional Params', 'topscorer-core' ),
			'options'    => array(
				''    => esc_html__( 'No', 'topscorer-core' ),
				'id'  => esc_html__( 'Post IDs', 'topscorer-core' ),
				'tax' => esc_html__( 'Taxonomy Slug', 'topscorer-core' ),
			),
			'group'      => $group
		) );
		$this->set_option( array(
			'field_type' => 'text',
			'name'       => 'post_ids',
			'title'      => esc_html__( 'Posts IDs (separate with comma)', 'topscorer-core' ),
			'group'      => $group,
			'dependency' => array(
				'show' => array(
		...
		if ( ! empty( $taxonomies_formatted ) ) {
			$this->set_option( array(
				'field_type' => 'select',
				'name'       => 'tax',
				'title'      => esc_html__( 'Taxonomy', 'topscorer-core' ),
			...
			$this->set_option( array(
				'field_type' => 'text',
				'name'       => 'tax_slug',
				'title'      => esc_html__( 'Taxonomy Slug', 'topscorer-core' ),
				'group'      => $group,
				'dependency' => array(
					'show' => array(
						'additional_params' => array(
			...
			$this->set_option( array(
				'field_type'  => 'text',
				'name'        => 'tax__in',
				'title'       => esc_html__( 'Taxonomy Ids', 'topscorer-core' ),
				'description' => esc_html__( 'Separate taxonomy ids with commas', 'topscorer-core' ),
				'group'       => $group,
				'dependency'  => array(
					'show' => array(
						'additional_params' => array(
	...
	public function map_layout_options( $params = array() ) {
		...
		if ( empty( $exclude_option ) || ( ! empty( $exclude_option ) && ! in_array( 'text_transform', $exclude_option ) ) ) {
			$this->set_option( array(
				'field_type' => 'select',
				'name'       => 'text_transform',
				'title'      => esc_html__( 'Title Text Transform', 'topscorer-core' ),
	...
	public function map_additional_options( $params = array() ) {
		...
		if ( ! $exclude_filter ) {
			$this->set_option( array(
				'field_type'    => 'select',
				'name'          => 'enable_filter',
				'title'         => esc_html__( 'Enable Filter', 'topscorer-core' ),
				'description'   => esc_html__( 'Enabling this option will show categories filer feature above list', 'topscorer-core' ),
				'options'       => topscorer_core_get_select_type_options_pool( 'no_yes' ),
				'default_value' => '',
				'group'         => esc_html__( 'Additional', 'topscorer-core' )
			) );
		}
		...
		if ( ! $exclude_pagination ) {
			$this->set_option( array(
				...
				'title'         => esc_html__( 'Pagination', 'topscorer-core' ),
				'options'       => topscorer_core_get_select_type_options_pool( 'pagination_type' ),
			...
			$this->set_option( array(
				...
				'title'         => esc_html__( 'Load More Top Margin (px or %)', 'topscorer-core' ),
				'default_value' => '',
				'group'         => esc_html__( 'Additional', 'topscorer-core' ),
				'dependency'    => array(
					'show' => array(
	...
	public function load_assets() {
		do_action( 'topscorer_core_action_list_shortcodes_load_assets', $this->get_atts() );
	}
