...
if ( ! function_exists( 'topscorer_core_add_icon_with_text_shortcode' ) ) {
	/**
	 ...
	 * @return array
	 */
	function topscorer_core_add_icon_with_text_shortcode( $shortcodes ) {
		$shortcodes[] = 'TopScorerCoreIconWithTextShortcode';
		
		return $shortcodes;
	}
	
	add_filter( 'topscorer_core_filter_register_shortcodes', 'topscorer_core_add_icon_with_text_shortcode' );
}

if ( class_exists( 'TopScorerCoreShortcode' ) ) {
	class TopScorerCoreIconWithTextShortcode extends TopScorerCoreShortcode {
		
		public function __construct() {
			$this->set_layouts( apply_filters( 'topscorer_core_filter_icon_with_text_layouts', array() ) );
			
			$options_map = topscorer_core_get_variations_options_map( $this->get_layouts() );
			$default_value = $options_map['default_value'];
			
			$this->set_extra_options( apply_filters( 'topscorer_core_filter_icon_with_text_extra_options', array(), $default_value ) );
			
			parent::__construct();
		}
		
		public function map_shortcode() {
			$this->set_shortcode_path( TOPSCORER_CORE_SHORTCODES_URL_PATH . '/icon-with-text' );
			$this->set_base( 'topscorer_core_icon_with_text' );
			$this->set_name( esc_html__( 'Icon With Text', 'topscorer-core' ) );
			$this->set_description( esc_html__( 'Shortcode that adds icon with text element', 'topscorer-core' ) );
		...
		public function render( $options, $content = null ) {
			...
			return topscorer_core_get_template_part( 'shortcodes/icon-with-text', 'variations/' . $atts['layout'] . '/templates/' . $atts['layout'], '', $atts );
		}
		...
		private function get_holder_classes( $atts ) {
			...
			$holder_classes = apply_filters( 'topscorer_core_filter_icon_with_text_variation_classes', $holder_classes, $atts );
