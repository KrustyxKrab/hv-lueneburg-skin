...
if ( ! function_exists( 'topscorer_core_add_image_gallery_shortcode' ) ) {
	/**
	 ...
	 * @return array
	 */
	function topscorer_core_add_image_gallery_shortcode( $shortcodes ) {
		$shortcodes[] = 'TopScorerCoreImageGalleryShortcode';
		
		return $shortcodes;
	}
	
	add_filter( 'topscorer_core_filter_register_shortcodes', 'topscorer_core_add_image_gallery_shortcode' );
}

if ( class_exists( 'TopScorerCoreListShortcode' ) ) {
	class TopScorerCoreImageGalleryShortcode extends TopScorerCoreListShortcode {
		
		public function map_shortcode() {
			$this->set_shortcode_path( TOPSCORER_CORE_SHORTCODES_URL_PATH . '/image-gallery' );
			$this->set_base( 'topscorer_core_image_gallery' );
			$this->set_name( esc_html__( 'Image Gallery', 'topscorer-core' ) );
			$this->set_description( esc_html__( 'Shortcode that adds image gallery element', 'topscorer-core' ) );
			...
			$this->set_option( array(
				...
				'name'       => 'images',
				'multiple'   => 'yes',
				'title'      => esc_html__( 'Images', 'topscorer-core' ),
			) );
			$this->set_option( array(
				'field_type' => 'text',
				'name'       => 'image_size',
			...
			$this->map_list_options( array(
				...
				'group'            => esc_html__( 'Gallery Settings', 'topscorer-core' )
			) );
		}
		...
		public function render( $options, $content = null ) {
			...
			return topscorer_core_get_template_part( 'shortcodes/image-gallery', 'templates/image-gallery', $atts['behavior'], $atts );
		}
