...
if ( ! function_exists( 'topscorer_core_add_info_section_shortcode' ) ) {
	/**
	 ...
	 * @return array
	 */
	function topscorer_core_add_info_section_shortcode( $shortcodes ) {
		$shortcodes[] = 'TopScorerCoreInfoSectionShortcode';
		
		return $shortcodes;
	}
	
	add_filter( 'topscorer_core_filter_register_shortcodes', 'topscorer_core_add_info_section_shortcode' );
}

if ( class_exists( 'TopScorerCoreShortcode' ) ) {
	class TopScorerCoreInfoSectionShortcode extends TopScorerCoreShortcode {
		
		public function __construct() {
			$this->set_layouts( apply_filters( 'topscorer_core_filter_info_section_layouts', array() ) );
			
			$options_map = topscorer_core_get_variations_options_map( $this->get_layouts() );
			$default_value = $options_map['default_value'];
			
			$this->set_extra_options( apply_filters( 'topscorer_core_filter_info_section_extra_options', array(), $default_value ) );
			
			parent::__construct();
		}
		
		public function map_shortcode() {
			$this->set_shortcode_path( TOPSCORER_CORE_SHORTCODES_URL_PATH . '/info-section' );
			$this->set_base( 'topscorer_core_info_section' );
			$this->set_name( esc_html__( 'Info Section', 'topscorer-core' ) );
			$this->set_description( esc_html__( 'Shortcode that adds info section element', 'topscorer-core' ) );
		...
		public function render( $options, $content = null ) {
			...
			$atts = apply_filters( 'topscorer_core_filter_info_section_variation_atts', $atts );
			
			return topscorer_core_get_template_part( 'shortcodes/info-section', 'variations/' . $atts['layout'] . '/templates/' . $atts['layout'], '', $atts );
		}
		...
		private function get_holder_classes( $atts ) {
			...
			$holder_classes = apply_filters( 'topscorer_core_filter_info_section_variation_classes', $holder_classes, $atts );
