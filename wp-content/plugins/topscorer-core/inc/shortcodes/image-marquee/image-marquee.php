...
if ( ! function_exists( 'topscorer_core_add_image_marquee_shortcode' ) ) {
	/**
	 ...
	 * @return array
	 */
	function topscorer_core_add_image_marquee_shortcode( $shortcodes ) {
		$shortcodes[] = 'TopScorerCoreImageMarqueeShortcode';
		
		return $shortcodes;
	}
	
	add_filter( 'topscorer_core_filter_register_shortcodes', 'topscorer_core_add_image_marquee_shortcode' );
}

if ( class_exists( 'TopScorerCoreShortcode' ) ) {
	class TopScorerCoreImageMarqueeShortcode extends TopScorerCoreShortcode {
		
		public function __construct() {
			$this->set_layouts( apply_filters( 'topscorer_core_filter_image_marquee_layouts', array() ) );
			$this->set_extra_options( apply_filters( 'topscorer_core_filter_image_marquee_extra_options', array() ) );
			
			parent::__construct();
		}
		
		public function map_shortcode() {
			$this->set_shortcode_path( TOPSCORER_CORE_SHORTCODES_URL_PATH . '/image-marquee' );
			$this->set_base( 'topscorer_core_image_marquee' );
			$this->set_name( esc_html__( 'Image Marquee', 'topscorer-core' ) );
			$this->set_description( esc_html__( 'Shortcode that adds Image Marquee element', 'topscorer-core' ) );
			...
			$this->set_option( array(
				'field_type'    => 'text',
				'name'          => 'duration',
				'title'         => esc_html__( 'Animation Duration (Seconds)', 'topscorer-core' ),
				'default_value' => '20',
			) );
			$this->set_option( array(
				'field_type' => 'select',
		...
		public function render( $options, $content = null ) {
			...
			return topscorer_core_get_template_part( 'shortcodes/image-marquee', 'variations/' . $atts['layout'] . '/templates/' . $atts['layout'], '', $atts );
		}
