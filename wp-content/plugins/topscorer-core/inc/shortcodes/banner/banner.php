...
if ( ! function_exists( 'topscorer_core_add_banner_shortcode' ) ) {
	/**
	 ...
	 * @return array
	 */
	function topscorer_core_add_banner_shortcode( $shortcodes ) {
		$shortcodes[] = 'TopScorerCoreBannerShortcode';

		return $shortcodes;
	}

	add_filter( 'topscorer_core_filter_register_shortcodes', 'topscorer_core_add_banner_shortcode' );
}

if ( class_exists( 'TopScorerCoreShortcode' ) ) {
	class TopScorerCoreBannerShortcode extends TopScorerCoreShortcode {

		public function __construct() {
			$this->set_layouts( apply_filters( 'topscorer_core_filter_banner_layouts', array() ) );
			$this->set_extra_options( apply_filters( 'topscorer_core_filter_banner_extra_options', array() ) );

			parent::__construct();
		}

		public function map_shortcode() {
			$this->set_shortcode_path( TOPSCORER_CORE_SHORTCODES_URL_PATH . '/banner' );
			$this->set_base( 'topscorer_core_banner' );
			$this->set_name( esc_html__( 'Banner', 'topscorer-core' ) );
			$this->set_description( esc_html__( 'Shortcode that adds banner element', 'topscorer-core' ) );
			...
			$this->set_option( array(
				'field_type'    => 'select',
				'name'          => 'text_behaviour',
				'title'         => esc_html__( 'Text Behaviour', 'topscorer-core' ),
				'options'       => array(
					'displayed-on-hover' => esc_html__( 'Displayed on Hover', 'topscorer-core' ),
					'permanently-displayed'          => esc_html__( 'Permanently Displayed', 'topscorer-core' ),
				),
			) );
			$this->set_option( array(
				'field_type' => 'image',
			...
			$this->set_option( array(
				...
				'title'      => esc_html__( 'Subtitle Margin Top', 'topscorer-core' ),
				'group'      => esc_html__( 'Content', 'topscorer-core' )
			) );
			$this->set_option( array(
				'field_type' => 'text',
			...
			$this->import_shortcode_options( array(
				'shortcode_base'    => 'topscorer_core_button',
				'exclude'           => array( 'custom_class', 'link', 'target' ),
				'additional_params' => array(
					'group'      => esc_html__( 'Button', 'topscorer-core' ),
					'dependency' => array(
						'show' => array(
							'layout' => array(
								'values'        => 'link-button',
		...
		public function render( $options, $content = null ) {
			...
			$atts['subtitle']        = $this->get_split_text( $atts['subtitle'] );
			
			return topscorer_core_get_template_part( 'shortcodes/banner', 'variations/' . $atts['layout'] . '/templates/' . $atts['layout'], '', $atts );
		}
		...
		private function get_holder_classes( $atts ) {
			...
			$holder_classes[] = ! empty ( $atts['text_behaviour'] ) ? 'qodef-behaviour--' . $atts['text_behaviour'] : '';
