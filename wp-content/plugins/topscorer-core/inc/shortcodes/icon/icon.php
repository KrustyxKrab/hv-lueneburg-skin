...
if ( ! function_exists( 'topscorer_core_add_icon_shortcode' ) ) {
	/**
	 ...
	 * @return array
	 */
	function topscorer_core_add_icon_shortcode( $shortcodes ) {
		$shortcodes[] = 'TopScorerCoreIconShortcode';
		
		return $shortcodes;
	}
	
	add_filter( 'topscorer_core_filter_register_shortcodes', 'topscorer_core_add_icon_shortcode', 8 );
}

if ( class_exists( 'TopScorerCoreShortcode' ) ) {
	class TopScorerCoreIconShortcode extends TopScorerCoreShortcode {
		
		public function map_shortcode() {
			$this->set_shortcode_path( TOPSCORER_CORE_SHORTCODES_URL_PATH . '/icon' );
			$this->set_base( 'topscorer_core_icon' );
			$this->set_name( esc_html__( 'Icon', 'topscorer-core' ) );
			$this->set_description( esc_html__( 'Shortcode that adds icon element', 'topscorer-core' ) );
			...
			$this->set_option( array(
				...
				'options'    => array(
					...
					'tiny'    => esc_html__( 'Tiny', 'topscorer-core' ),
					'small'   => esc_html__( 'Small', 'topscorer-core' ),
					'medium'  => esc_html__( 'Medium', 'topscorer-core' ),
					'large'   => esc_html__( 'Large', 'topscorer-core' ),
					'huge'    => esc_html__( 'Huge', 'topscorer-core' ),
				)
			) );
			$this->set_option( array(
				'field_type' => 'text',
				'name'       => 'custom_size',
				'title'      => esc_html__( 'Custom Size', 'topscorer-core' ),
			) );
			$this->set_option( array(
				'field_type'    => 'select',
				'name'          => 'icon_layout',
				...
				'options'       => array(
					'normal' => esc_html__( 'Normal', 'topscorer-core' ),
					'circle' => esc_html__( 'Circle', 'topscorer-core' ),
					'square' => esc_html__( 'Square', 'topscorer-core' )
				),
			...
			$this->set_option( array(
				'field_type' => 'text',
				'name'       => 'border_radius',
				'title'      => esc_html__( 'Border Radius', 'topscorer-core' ),
				'dependency' => array(
					'show' => array(
						'icon_layout' => array(
							'values'        => 'square',
			...
			$this->set_option( array(
				'field_type' => 'text',
				'name'       => 'shape_size',
				'title'      => esc_html__( 'Shape Size', 'topscorer-core' ),
				'dependency' => array(
					'show' => array(
						'icon_layout' => array(
			...
			$this->set_option( array(
				'field_type' => 'text',
				'name'       => 'border_width',
				'title'      => esc_html__( 'Border Width', 'topscorer-core' ),
				'dependency' => array(
					'show' => array(
						'icon_layout' => array(
							'values'        => array( 'circle', 'square' ),
		...
		public static function call_shortcode( $params ) {
			$html = qode_framework_call_shortcode( 'topscorer_core_icon', $params );
			$html = str_replace( "\n", '', $html );
		...
		public function render( $options, $content = null ) {
			...
			return topscorer_core_get_template_part( 'shortcodes/icon', 'templates/icon', '', $atts );
		}
