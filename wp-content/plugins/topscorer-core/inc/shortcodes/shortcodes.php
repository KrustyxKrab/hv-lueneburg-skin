...
if ( ! class_exists( 'TopScorerCoreShortcodes' ) ) {
	class TopScorerCoreShortcodes {
		private static $instance;
		private $allowed_shortcodes = array();

		public function __construct() {
			$check_code = class_exists( 'TopScorerCoreDashboard' ) ? TopScorerCoreDashboard::get_instance()->get_code() : true;
			...
			if ( ! empty( $check_code ) ) {
				...
				add_action( 'qode_framework_action_before_shortcodes_register', array(
					$this,
					'include_shortcode_classes'
				), 5 );
				...
				add_action( 'qode_framework_action_before_widgets_register', array(
					$this,
					'include_shortcodes_widget'
				) );
				
				// Include shortcodes media fields
				add_action( 'qode_framework_action_custom_media_fields', array(
					$this,
					'include_shortcodes_media_fields'
				) );
			}
		}
		...
		function init() {
			...
			include_once TOPSCORER_CORE_SHORTCODES_PATH . '/dashboard/customizer/shortcodes-customizer-options.php';
		}
		...
		function set_enabled_shortcodes() {
			...
			foreach ( glob( TOPSCORER_CORE_SHORTCODES_PATH . '/*', GLOB_ONLYDIR ) as $shortcode ) {
				...
				if ( basename( $shortcode ) !== 'dashboard' ) {
					$is_disabled = topscorer_core_performance_get_option_value( $shortcode, 'topscorer_core_performance_shortcode_' );
		...
		function include_shortcode_classes() {
			include_once TOPSCORER_CORE_SHORTCODES_PATH . '/shortcode.php';
			include_once TOPSCORER_CORE_SHORTCODES_PATH . '/list-shortcode.php';
		}
		...
		function register_shortcodes() {
			...
			$shortcodes     = apply_filters( 'topscorer_core_filter_register_shortcodes', $shortcodes = array() );
	...
	TopScorerCoreShortcodes::get_instance();
}