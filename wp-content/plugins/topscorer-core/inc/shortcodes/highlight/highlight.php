...
if ( ! function_exists( 'topscorer_core_add_highlight_shortcode' ) ) {
	/**
	 ...
	 * @return array
	 */
	function topscorer_core_add_highlight_shortcode( $shortcodes ) {
		$shortcodes[] = 'TopScorerCoreHighlightShortcode';

		return $shortcodes;
	}

	add_filter( 'topscorer_core_filter_register_shortcodes', 'topscorer_core_add_highlight_shortcode' );
}

if ( class_exists( 'TopScorerCoreShortcode' ) ) {
	class TopScorerCoreHighlightShortcode extends TopScorerCoreShortcode {

		public function map_shortcode() {
			$this->set_shortcode_path( TOPSCORER_CORE_SHORTCODES_URL_PATH . '/highlight' );
			$this->set_base( 'topscorer_core_highlight' );
			$this->set_name( esc_html__( 'Highlight', 'topscorer-core' ) );
			$this->set_description( esc_html__( 'Shortcode that displays highlight with provided parameters', 'topscorer-core' ) );
			...
			$this->set_option( array(
				'field_type'    => 'select',
				'name'          => 'text_tag',
				'title'         => esc_html__( 'Text Tag', 'topscorer-core' ),
			...
			$this->set_option( array(
				...
				'title'         => esc_html__( 'Disable Text Line Break', 'topscorer-core' ),
				'description'   => esc_html__( 'Enabling this option will disable text line breaks for screen size 1024 and lower', 'topscorer-core' ),
			...
			$this->set_option( array(
				...
				'title'       => esc_html__( 'Highlight Text Position', 'topscorer-core' ),
				'description' => esc_html__( 'Enter the positions of the words you would like to display as "highlight" text. Separate start and end positions with commas (e.g. if you would like to wrap from fifth to ninth words, you would enter "5,9") or if you want to highlight whole text fill -1', 'topscorer-core' ),
			) );
			$this->set_option( array(
				'field_type' => 'color',
				'name'       => 'highlight_color',
				'title'      => esc_html__( 'Highlight Text Color', 'topscorer-core' )
			) );
			$this->set_option( array(
				'field_type' => 'color',
				'name'       => 'highlight_background_color',
				'title'      => esc_html__( 'Highlight Text Background Color', 'topscorer-core' )
			) );
			$this->set_option( array(
				'field_type'    => 'text',
				'name'          => 'link',
		...
		public function render( $options, $content = null ) {
			...
			return topscorer_core_get_template_part( 'shortcodes/highlight', 'templates/highlight', '', $atts );
		}
