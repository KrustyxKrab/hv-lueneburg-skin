...
if ( ! class_exists( 'TopScorerCoreContactForm7' ) ) {
	class TopScorerCoreContactForm7 {
		...
		function include_widgets() {
			foreach ( glob( TOPSCORER_CORE_INC_PATH . '/contact-form-7/widgets/*/include.php' ) as $widget ) {
				include_once $widget;
			}
		}
		...
		function override_templates() {
			...
			add_action( 'wpcf7_init', 'topscorer_core_cf7_add_submit_form_tag' );
		}
	}
	
	TopScorerCoreContactForm7::get_instance();
}