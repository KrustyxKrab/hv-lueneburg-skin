...
if ( ! function_exists( 'topscorer_core_sportspress_add_countdown_shortcode' ) ) {
	/**
	 ...
	 * @return array
	 */
	function topscorer_core_sportspress_add_countdown_shortcode( $shortcodes ) {
		$shortcodes[] = 'TopScorerCoreSportsPressCountdownShortcode';

		return $shortcodes;
	}

	add_filter( 'topscorer_core_filter_register_shortcodes', 'topscorer_core_sportspress_add_countdown_shortcode' );
}

if ( class_exists( 'TopScorerCoreShortcode' ) ) {
	class TopScorerCoreSportsPressCountdownShortcode extends TopScorerCoreShortcode {

		public function map_shortcode() {
			$this->set_shortcode_path( TOPSCORER_CORE_INC_URL_PATH . '/sportspress/shortcodes/sportspress-countdown' );
			$this->set_base( 'topscorer_core_sportspress_countdown' );
			$this->set_name( esc_html__( 'Countdown', 'topscorer-core' ) );
			$this->set_description( esc_html__( 'Shortcode that displays a countdown clock to the single event', 'topscorer-core' ) );
			...
			$this->set_option( array(
				'field_type'    => 'select',
				'name'          => 'show_timer',
				'title'         => esc_html__( 'Show Timer', 'topscorer-core' ),
				'options'       => topscorer_core_get_select_type_options_pool( 'yes_no', false ),
				'default_value' => 'yes',
				'dependency'    => array(
					'show' => array(
						'event_type' => array(
							'values'        => array( 'next', 'upcoming' ),
							'default_value' => 'next'
						)
					)
				)
			...
			$this->set_option( array(
				'field_type' => 'select',
				'name'       => TOPSCORER_CORE_SPORTSPRESS_SHORTCODE_PREFIX . 'live',
				'title'      => esc_html__( 'Live Timer', 'topscorer-core' ),
		...
		public static function call_shortcode( $params ) {
			$html = qode_framework_call_shortcode( 'topscorer_core_sportspress_countdown', $params );
			$html = str_replace( "\n", '', $html );
		...
		private function get_holder_classes( $atts ) {
			...
			$holder_classes[] = 'qodef-sportspress-countdown';
			...
			$holder_classes[] = ( $atts['show_timer'] === 'yes' ) ? 'qodef-sportspress-countdown--show-timer' : 'qodef-sportspress-countdown--hide-timer';
