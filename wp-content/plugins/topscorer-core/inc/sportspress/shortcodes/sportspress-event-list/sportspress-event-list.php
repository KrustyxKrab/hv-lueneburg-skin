...
if ( ! function_exists( 'topscorer_core_sportspress_add_event_list_shortcode' ) ) {
	/**
	 ...
	 * @return array
	 */
	function topscorer_core_sportspress_add_event_list_shortcode( $shortcodes ) {
		$shortcodes[] = 'TopScorerCoreSportsPressEventListShortcode';

		return $shortcodes;
	}

	add_filter( 'topscorer_core_filter_register_shortcodes', 'topscorer_core_sportspress_add_event_list_shortcode' );
}

if ( class_exists( 'TopScorerCoreShortcode' ) ) {
	class TopScorerCoreSportsPressEventListShortcode extends TopScorerCoreShortcode {

		public function map_shortcode() {
			$this->set_shortcode_path( TOPSCORER_CORE_INC_URL_PATH . '/sportspress/shortcodes/sportspress-event-list' );
			$this->set_base( 'topscorer_core_sportspress_event_list' );
			$this->set_name( esc_html__( 'Event List', 'topscorer-core' ) );
			$this->set_description( esc_html__( 'Shortcode that displays a list of events from a calendar', 'topscorer-core' ) );
			...
			$this->set_option( array(
				'field_type' => 'select',
				'name'       => TOPSCORER_CORE_SPORTSPRESS_SHORTCODE_PREFIX . 'link_venues',
				'title'      => esc_html__( 'Link Venues', 'topscorer-core' ),
				'options'    => topscorer_core_get_select_type_options_pool( '0_1', false )
			) );
			$this->set_option( array(
				'field_type' => 'select',
		...
		public static function call_shortcode( $params ) {
			$html = qode_framework_call_shortcode( 'topscorer_core_sportspress_event_list', $params );
			$html = str_replace( "\n", '', $html );
		...
		private function get_holder_classes( $atts ) {
			...
			$holder_classes[] = 'qodef-sportspress-event-list';
