...
if ( ! function_exists( 'topscorer_core_sportspress_add_event_performance_shortcode' ) ) {
	/**
	 ...
	 * @return array
	 */
	function topscorer_core_sportspress_add_event_performance_shortcode( $shortcodes ) {
		$shortcodes[] = 'TopScorerCoreSportsPressEventPerformanceShortcode';

		return $shortcodes;
	}

	add_filter( 'topscorer_core_filter_register_shortcodes', 'topscorer_core_sportspress_add_event_performance_shortcode' );
}

if ( class_exists( 'TopScorerCoreShortcode' ) ) {
	class TopScorerCoreSportsPressEventPerformanceShortcode extends TopScorerCoreShortcode {

		public function map_shortcode() {
			$this->set_shortcode_path( TOPSCORER_CORE_INC_URL_PATH . '/sportspress/shortcodes/sportspress-event-performance' );
			$this->set_base( 'topscorer_core_sportspress_event_performance' );
			$this->set_name( esc_html__( 'Event Performance', 'topscorer-core' ) );
			$this->set_description( esc_html__( 'Shortcode that displays team performance of a single event', 'topscorer-core' ) );
		...
		public static function call_shortcode( $params ) {
			$html = qode_framework_call_shortcode( 'topscorer_core_sportspress_event_performance', $params );
			$html = str_replace( "\n", '', $html );
		...
		public function render( $options, $content = null ) {
			...
			$atts['shortcode']                                         = 'event_performance';
		...
		private function get_holder_classes( $atts ) {
			...
			$holder_classes[] = 'qodef-sportspress-event-performance';
