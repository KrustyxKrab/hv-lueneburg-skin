...
if ( ! function_exists( 'topscorer_core_sportspress_add_event_details_shortcode' ) ) {
	/**
	 ...
	 * @return array
	 */
	function topscorer_core_sportspress_add_event_details_shortcode( $shortcodes ) {
		$shortcodes[] = 'TopScorerCoreSportsPressEventDetailsShortcode';

		return $shortcodes;
	}

	add_filter( 'topscorer_core_filter_register_shortcodes', 'topscorer_core_sportspress_add_event_details_shortcode' );
}

if ( class_exists( 'TopScorerCoreShortcode' ) ) {
	class TopScorerCoreSportsPressEventDetailsShortcode extends TopScorerCoreShortcode {

		public function map_shortcode() {
			$this->set_shortcode_path( TOPSCORER_CORE_INC_URL_PATH . '/sportspress/shortcodes/sportspress-event-details' );
			$this->set_base( 'topscorer_core_sportspress_event_details' );
			$this->set_name( esc_html__( 'Event Details', 'topscorer-core' ) );
			$this->set_description( esc_html__( 'Shortcode that displays details of a single event', 'topscorer-core' ) );
		...
		public static function call_shortcode( $params ) {
			$html = qode_framework_call_shortcode( 'topscorer_core_sportspress_event_details', $params );
			$html = str_replace( "\n", '', $html );
		...
		private function get_holder_classes( $atts ) {
			...
			$holder_classes[] = 'qodef-sportspress-event-details';
