<?php
class CoversHeaderSearch extends TopScorerCoreSearch {
	...
	public function return_template(){
		return topscorer_core_template_part( 'search/layouts/' . $this->search_layout, 'templates/' . $this->search_layout );
	}
	
	public function load_template() {
		if ( is_active_widget( false, false, 'topscorer_core_search_opener' ) ) {
			
			$actions         = array();
			$sidebars        = qode_framework_get_widget_sidebars( 'topscorer_core_search_opener' );
			...
			foreach ( $sidebars as $sidebar ) {
				if ( $sidebar == 'qodef-header-widget-area-one' || $sidebar == 'qodef-header-widget-area-two' ) {
					$actions[] = 'topscorer_action_after_page_header_inner';
					$actions[] = 'topscorer_core_action_after_sticky_header';
				} else if ( $sidebar == 'qodef-mobile-header-widget-area' ) {
					$actions[] = 'topscorer_action_after_page_mobile_header_inner';
				} else if ( $sidebar == 'qodef-top-area-left' || $sidebar == 'qodef-top-area-right' ) {
					$actions[] = 'topscorer_core_action_after_top_area';
