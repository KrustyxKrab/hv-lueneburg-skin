...
if ( ! function_exists( 'topscorer_core_add_top_area_options' ) ) {
	function topscorer_core_add_top_area_options( $page ) {

		$top_area_section = $page->add_section_element(
			array(
				'name'        => 'qodef_top_area_section',
				'title'       => esc_html__( 'Top Area', 'topscorer-core' ),
				'description' => esc_html__( 'Options related to top area', 'topscorer-core' ),
				'dependency' => array(
					'hide' => array(
	...
	add_action( 'topscorer_core_action_after_header_options_map', 'topscorer_core_add_top_area_options', 20, 2);
}