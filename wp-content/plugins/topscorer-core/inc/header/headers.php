...
class TopScorerCoreHeaders {
	...
	public function __construct() {
		...
		add_action( 'topscorer_action_after_page_header_inner', array( $this, 'scroll_appearance' ) );
...
//		foreach ( glob( TOPSCORER_CORE_INC_PATH . '/header/dashboard/*/*.php' ) as $admin ) {
//			include_once $admin;
//		}
//		foreach ( glob( TOPSCORER_CORE_INC_PATH . '/header/layouts/*/include.php' ) as $layout ) {
//			include_once $layout;
//		}
//		foreach ( glob( TOPSCORER_CORE_INC_PATH . '/header/scroll-appearance/*/include.php' ) as $appearance ) {
//			include_once $appearance;
//		}
//		include_once TOPSCORER_CORE_INC_PATH . '/header/top-area/include.php';
		...
		foreach ( glob( TOPSCORER_CORE_INC_PATH . '/header/*/include.php' ) as $header_part ) {
			include_once $header_part;
		}
	}

	function set_variables() {
		$layout_meta = topscorer_core_get_post_value_through_levels( 'qodef_header_layout' );
		$layouts     = apply_filters( 'topscorer_core_filter_register_header_layouts', $header_layouts_option = array() );
	...
	function render_template() {
		...
		if ( ! empty( $header_object ) ) {
			$template_hook = $header_object->overriding_whole_header ? 'topscorer_filter_header_template' : 'topscorer_filter_header_content_template';
	...
	function add_body_classes( $classes ) {
		$header_layout            = topscorer_core_get_post_value_through_levels( 'qodef_header_layout' );
		$header_skin              = topscorer_core_get_post_value_through_levels( 'qodef_header_skin' );
	...
	function scroll_appearance() {
		...
		if ( ! empty( $header_object ) ) {
			$appearance_type = topscorer_core_get_post_value_through_levels( 'qodef_header_scroll_appearance' );

			if ( file_exists( TOPSCORER_CORE_HEADER_LAYOUTS_PATH . '/' . $header_object->slug . '/templates/' . $appearance_type . '.php' ) ) {
				$scroll_appearance_layout = 'layouts/' . $header_object->slug;
			} else {
				$scroll_appearance_layout = 'scroll-appearance/' . $appearance_type;
			}

			topscorer_core_template_part( 'header/' . $scroll_appearance_layout, 'templates/' . $appearance_type, '', array() );
		}
	}
	...
	function add_header_widget_areas() {
		register_sidebar(
			array(
				...
				'name'          => esc_html__( 'Header - Area One', 'topscorer-core' ),
				'description'   => esc_html__( 'Widgets added here will appear in header widget area one', 'topscorer-core' ),
		...
		register_sidebar(
			array(
				...
				'name'          => esc_html__( 'Header - Area Two', 'topscorer-core' ),
				'description'   => esc_html__( 'Widgets added here will appear in header widget area two', 'topscorer-core' ),
		...
		do_action( 'topscorer_core_action_additional_header_widgets_area' );
	}
}

TopScorerCoreHeaders::get_instance();