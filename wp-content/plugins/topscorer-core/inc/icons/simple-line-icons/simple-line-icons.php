...
if ( ! function_exists( 'topscorer_core_add_simple_line_icons_to_collection' ) ) {
	function topscorer_core_add_simple_line_icons_to_collection( $icons ) {
		$icons[] = 'TopScorerCoreSimpleLineIconsPack';
		return $icons;
	}
	
	add_filter( 'qode_framework_filter_add_icon', 'topscorer_core_add_simple_line_icons_to_collection' );
}

if ( class_exists( 'QodeFrameworkIconPack' ) ) {
	class TopScorerCoreSimpleLineIconsPack extends QodeFrameworkIconPack {
		...
		public function get_style_url() {
			return TOPSCORER_CORE_INC_URL_PATH . '/icons/' . $this->get_base() . '/assets/css/' . $this->get_base() . '.min.css';
		}
