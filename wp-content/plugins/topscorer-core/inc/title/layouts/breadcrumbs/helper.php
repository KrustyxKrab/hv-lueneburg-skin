...
if ( ! function_exists( 'topscorer_core_register_breadcrumbs_title_layout' ) ) {
	function topscorer_core_register_breadcrumbs_title_layout( $layouts ) {
		$layouts['breadcrumbs'] = 'TopScorerCoreBreadcrumbsTitle';

		return $layouts;
	}

	add_filter( 'topscorer_core_filter_register_title_layouts', 'topscorer_core_register_breadcrumbs_title_layout' );
}

if ( ! function_exists( 'topscorer_core_add_breadcrumbs_title_layout_option' ) ) {
	/**
	 ...
	 * @return array
	 */
	function topscorer_core_add_breadcrumbs_title_layout_option( $layouts ) {
		$layouts['breadcrumbs'] = esc_html__( 'Breadcrumbs', 'topscorer-core' );

		return $layouts;
	}

	add_filter( 'topscorer_core_filter_title_layout_options', 'topscorer_core_add_breadcrumbs_title_layout_option' );
}

if ( ! function_exists( 'topscorer_core_breadcrumbs' ) ) {
	/**
	 * Function that renders breadcrumbs html
	 */
	function topscorer_core_breadcrumbs() {
		$page_id = qode_framework_get_page_id();

		// Breadcrumbs label
		$labels = apply_filters( 'topscorer_core_filter_breadcrumbs_label', array(
			'home'        => esc_html__( 'Home', 'topscorer-core' ),
			'tag'         => esc_html__( 'Posts tagged "%s"', 'topscorer-core' ),
			'author'      => esc_html__( 'Posted by %s', 'topscorer-core' ),
			'search'      => esc_html__( 'Search results for "%s"', 'topscorer-core' ),
			'404'         => esc_html__( '404 - Page not found', 'topscorer-core' ),
			'query_paged' => esc_html__( '(Page %s)', 'topscorer-core' )
		) );

		// Breadcrumbs variables
		$settings = apply_filters( 'topscorer_core_filter_breadcrumbs_settings', array(
		...
		$breadcrumbs_html = '';
		if ( ! empty( $wrap ) ) {
			$breadcrumbs_html = $settings['wrap_before'] . $wrap . apply_filters( 'topscorer_core_filter_breadcrumbs_content', $wrap_child, $settings ) . $settings['wrap_after'];
		}

		echo apply_filters( 'topscorer_core_filter_breadcrumbs_template', $breadcrumbs_html );
	}
}