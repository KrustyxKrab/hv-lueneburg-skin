...
if ( ! function_exists( 'topscorer_core_dropdown_item_classes' ) ) {
	function topscorer_core_dropdown_item_classes( $classes, $item, $args, $depth ) {
		
		if ( $depth == 0 && in_array( 'menu-item-has-children', $item->classes ) ) {
			$mega_menu   = topscorer_core_get_option_value( 'nav-menu', 'qodef-enable-mega-menu', '', $item->ID );
			...
			if ( $mega_menu_e ) {
				...
				add_filter( 'topscorer_core_filter_drop_down_second_inner_classes', function ( $innerClasses ) {
					$grid_class = false;
					$full_width = topscorer_core_get_post_value_through_levels( 'qodef_wide_dropdown_full_width' );
					$grid = topscorer_core_get_post_value_through_levels( 'qodef_wide_dropdown_content_grid' );
					
					if ( $grid == 'yes' || $full_width == 'no') {
						$grid_class = true;
					}
					
					$grid_class = apply_filters('topscorer_core_filter_drop_down_grid', $grid_class);
	...
	add_filter( 'nav_menu_css_class', 'topscorer_core_dropdown_item_classes', 11, 4 );
}

if ( ! function_exists( 'topscorer_core_add_nav_menu_body_classes' ) ) {
	function topscorer_core_add_nav_menu_body_classes( $classes ) {
		$full_width = topscorer_core_get_post_value_through_levels( 'qodef_wide_dropdown_full_width' );
		$appearance = topscorer_core_get_post_value_through_levels( 'qodef_dropdown_appearance' );
	...
	add_filter( 'body_class', 'topscorer_core_add_nav_menu_body_classes' );
}

if ( ! function_exists( 'topscorer_core_set_nav_menu_typography_styles' ) ) {
	/**
	 ...
	 * @return string
	 */
	function topscorer_core_set_nav_menu_typography_styles( $style ) {
		$scope = TOPSCORER_CORE_OPTIONS_NAME;
		
		$first_lvl_styles       = topscorer_core_get_typography_styles( $scope, 'qodef_nav_1st_lvl' );
		$first_lvl_hover_styles = topscorer_core_get_typography_hover_styles( $scope, 'qodef_nav_1st_lvl' );
		$second_lvl_styles      = topscorer_core_get_typography_styles( $scope, 'qodef_nav_2nd_lvl' );
		$second_lvl_wide_styles = topscorer_core_get_typography_styles( $scope, 'qodef_nav_2nd_lvl_wide' );
		$third_lvl_wide_styles  = topscorer_core_get_typography_styles( $scope, 'qodef_nav_3rd_lvl_wide' );
		...
		$first_lvl_active_color = topscorer_core_get_option_value( 'admin', 'qodef_nav_1st_lvl_active_color' );
		...
		$second_lvl_active_color = topscorer_core_get_option_value( 'admin', 'qodef_nav_2nd_lvl_active_color' );
		...
		$second_lvl_wide_active_color = topscorer_core_get_option_value( 'admin', 'qodef_nav_2nd_lvl_wide_active_color' );
		...
		$third_lvl_wide_active_color = topscorer_core_get_option_value( 'admin', 'qodef_nav_3rd_lvl_wide_active_color' );
	...
	add_filter( 'topscorer_filter_add_inline_style', 'topscorer_core_set_nav_menu_typography_styles' );
}